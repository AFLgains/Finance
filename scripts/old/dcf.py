...
def calc_time_series(stock_ticker_str: str, dates: List[str], statement: List[Dict], line_item: str) -> Dict[str,float]:
    return {dates[d]:statement[stock_ticker_str][d][dates[d]][line_item] for d in range(len(dates)) }

def calc_eff_tax_rate(stock_ticker_str: str) -> float:
    return 0.26

# Get the operating cashflow time series
cash_time_series = calc_time_series(stock_ticker_str, dates,cash,'totalCashFromOperatingActivities')
...
capex_time_series = calc_time_series(stock_ticker_str, dates,cash,'capitalExpenditures')

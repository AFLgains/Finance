...
def add_dcf(stock_name, price_history, min_date, max_date):
    ...
    dcf_quarterly_all = pd.DataFrame(
        {"formatted_date": pd.date_range(start=min_date, end=max_date)}
    )
    dcf_quarterly_all = dcf_quarterly_all.merge(
        dcf_quarterly, on="formatted_date", how="left"
    )
    ...
    price_history = pd.merge(
        price_history, dcf_quarterly_all, on="formatted_date", how="left"
    )
...
def add_key_metrics(stock_name, price_history, min_date, max_date):
    ...
    key_metrics_quarterly["formatted_date"] = pd.to_datetime(
        key_metrics_quarterly.index
    )
    ...
    key_metrics_quarterly_all = key_metrics_quarterly_all.merge(
        key_metrics_quarterly, on="formatted_date", how="left"
    )
    ...
    # Merge to the price history
    price_history = pd.merge(
        price_history, key_metrics_quarterly_all, on="formatted_date", how="left"
    )
...
def add_financial_statement_data(stock_name, price_history, min_date, max_date):
    ...
    balance_sheet = balance_sheet[
        ["Total assets", "Total current liabilities", "Goodwill and Intangible Assets"]
    ].astype(float)
    ...
    income_sheet["Preferred Dividends"] = income_sheet["Preferred Dividends"].astype(
        float
    )
    income_sheet["Weighted Average Shs Out"] = income_sheet[
        "Weighted Average Shs Out"
    ].astype(float)

    income_sheet["earnings"] = (
        income_sheet["Net Income"] - income_sheet["Preferred Dividends"]
    )
    income_sheet["EBIT"] = income_sheet["EBIT"].rolling(4).sum()  # Take rolling sum
    income_sheet["earnings"] = (
        income_sheet["earnings"].rolling(4).sum()
    )  # Take rolling sum
    income_sheet["EPS"] = (
        income_sheet["earnings"] / income_sheet["Weighted Average Shs Out"]
    )
    income_sheet["earnings_growth"] = (
        income_sheet["earnings"] - income_sheet["earnings"].shift(1)
    ) / income_sheet["earnings"].shift(1)
    income_sheet = income_sheet[["EPS", "EBIT", "earnings_growth"]]

    # ROCE
    total_financials = pd.merge(
        balance_sheet, income_sheet, on="formatted_date", how="left"
    )
    total_financials["ROCE"] = total_financials["EBIT"] / (
        total_financials["Total assets"]
        - total_financials["Total current liabilities"]
        - 0 * total_financials["Goodwill and Intangible Assets"]
    )

    # Expand to count for all days
    total_financials_all = pd.DataFrame(
        {"formatted_date": pd.date_range(start=min_date, end=max_date)}
    )
    total_financials_all = total_financials_all.merge(
        total_financials, on="formatted_date", how="left"
    )
    ...
    # Merge to the price history
    price_history = pd.merge(
        price_history, total_financials_all, on="formatted_date", how="left"
    )
...
def main(n_stocks=100):
    stock_data_dict = pickle.load(
        open("C:\Personalprojects\Finance\scripts\stock_history_price_data.pkl", "rb")
    )
    stock_tocker_str = [ticker for ticker in stock_data_dict.keys()]
    stock_tocker_str = random.sample(
        stock_tocker_str, min(n_stocks, len(stock_tocker_str))
    )
    ...
    for stock_name in stock_tocker_str:
        try:
            ...
            stock_data = (
                pd.DataFrame(stock_data_dict[stock_name]["prices"])
                .drop("date", axis=1)
    ...
    for i, st in enumerate(data_df):
        try:
            ...
            st.price_history = add_financial_statement_data(
                st.name, st.price_history, start_str, end_str
            )

            # Get rid of NANs in key fields #TODO: Make this better
            st.price_history = st.price_history.loc[
                ~np.isnan(st.price_history.DCF),
            ]
            st.price_history = st.price_history.loc[
                ~np.isnan(st.price_history.adjclose),
            ]
            st.price_history = st.price_history.loc[
                ~np.isnan(st.price_history.ROCE),
            ]
            st.price_history = st.price_history.loc[
                ~np.isnan(st.price_history.EPS),
            ]
            st.price_history = st.price_history.loc[
                st.price_history.EPS != float("inf"),
            ]

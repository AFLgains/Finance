...
from strategies.strategies import buy_and_hold, red_white_blue, DCF, ROC_PE, ROC_DCF
import random
...
data_df_list = [data_df.loc[data_df.Ticker==x,:].rename(columns ={"Date":"formatted_date","Adj. Close":"adjclose"}) for x in set(data_df.Ticker) if len(data_df.loc[data_df.Ticker==x,'Ticker'])>200]
...
print(f'{"Strategy":20}|{"Stock purchased":15}|{"E(return_annual)":16}|{"spread":10}|{"total_trades":15}|{"total_invested":15}|{"total_after":15}|{"overall_annualised_return":30}|{"batting_average":20}')
print(f'-----------------------------------------------------------------------------------------------------------------------------------------------------')
...
plt.hist([x.annualised_return_rate for x in buy_and_hold_results.evaluate()[0]],bins =20,alpha = 0.3)
plt.hist([x.annualised_return_rate for x in red_white_blue_results.evaluate()[0]],bins =20,alpha = 0.3)
plt.xlim([-1,0.4])
plt.show()

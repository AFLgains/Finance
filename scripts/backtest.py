...
from metrics.financial_metrics import add_pe_ratio, add_roce_ratio, add_net_income_postive_history, add_roa_ratio, add_price_to_book, add_net_income_to_book, add_current_assets_to_liabilities, add_net_income_to_book_g_limit_n_periods, add_earnings_growth_n_periods
from simfin.names import *
from strategies.strategies import ROC_PE, buy_and_hold, buy_and_hold_year, red_white_blue, MOD_LIL_BOOK, MOD_WARI_B
...
INCOME_COLUMNS = [FISCAL_YEAR, FISCAL_PERIOD, REVENUE, NET_INCOME, SHARES_DILUTED, GROSS_PROFIT, OPERATING_EXPENSES]
...
FINAL_COLUMNS = [TICKER, DATE, ADJ_CLOSE, VOLUME, FISCAL_YEAR, FISCAL_PERIOD, PE,P_BOOK, CURRENT_RATIO, REVENUE, MARKET_CAP, NET_INCOME,"year_first_day","net_income_positive_all_history","mean_earnings_growth_n_periods","net_income_to_book_g_limit_n_periods","net_income_to_book","roce_ratio","Return on Assets"]
...
def update_fundamental_data(
    ...
    return df_income, df_balance, df_cash, df_price, df_fin_signals, df_growth_signals, df_val_signals
...
def generate_metrics_after_join(data_no_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    data_no_metrics = add_net_income_to_book(
        data_no_metrics, net_income_col= NET_INCOME, Total_assets_col= TOTAL_ASSETS, Total_liabilities_col = TOTAL_LIABILITIES,
    )
    # Time based metrics
    data_no_metrics = add_net_income_to_book_g_limit_n_periods(
        data_no_metrics, net_income_to_book_col= "net_income_to_book", limit= 0.1, window = 12,
    )
...
def generate_buy_time(df_final_no_buy_time: pd.DataFrame) -> pd.DataFrame:
    ...
    data_df['year_first_day'] = data_df.groupby('year')['Date'].transform(lambda x: min(x))
...
def build_data_cube(run_configs: Configuration) -> pd.DataFrame:
    ...
    df_income, df_balance, df_cash, df_prices, df_fin_signals, df_growth_signals, df_val_signals = update_fundamental_data(hub, run_configs)
    df_income, df_balance, df_cash, df_prices, df_fin_signals, df_growth_signals, df_val_signals = select_data_columns(run_configs, df_income, df_balance, df_cash, df_prices, df_fin_signals, df_growth_signals, df_val_signals)
...
def validate(data_df: pd.DataFrame, stock_name: str, run_config: Configuration) -> bool:
    ...
    if len(data_df.iloc[:,0]) < min_records:
        ...
        logging.info(f"Removing {stock_name} because min price less than {min_price} dollars")
        return False
    elif min(data_df[VOLUME]) < min_volume:
        logging.info(f"Removing {stock_name} because min volume less than {min_volume} trades per day")
        return False
    else:
        return True
    ...
    for rev_limit in [5,10]:
        for roa_lower_limit in [0.15]:
            for stock_limit in [10, 20, 30]:
                MOD_LIL_BOOK_20 = test_strategy(MOD_LIL_BOOK, "MOD_LB_"+str(stock_limit)+"_"+str(roa_lower_limit)+"_"+str(rev_limit), stock_list, purchase_frequency=1,redistribute = True, stock_limit=stock_limit, roa_lower_limit = roa_lower_limit,min_revenue = rev_limit*1e8)
